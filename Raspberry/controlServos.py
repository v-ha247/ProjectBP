        ...
        if((move_vertically > self.min_movement or move_vertically < 0-self.min_movement) and
           (move_vertically < self.max_movement or move_vertically > 0-self.max_movement)):
            ...
            if tilt_servo > self.max_y:
                tilt_servo = self.max_y
            if tilt_servo < self.min_y:
                tilt_servo = self.min_y
                
        if self.moving_x:      
            t_pan = Thread(target=self.smooth_pan, args=(old_angle_x,pan_servo))
            t_pan.daemon = True
            t_pan.start()
        
        if self.moving_y: 
            t_tilt = Thread(target=self.smooth_tilt, args=(old_angle_y,tilt_servo))
            t_tilt.daemon = True
            t_tilt.start()
            
    def smooth_pan(self, angle, move_to):
        print(f'pan, angle - {angle}, move_to - {move_to}')
        if angle < move_to:
            while angle < move_to:
                angle += self.step_movement
                if angle <= self.max_x:
                    self.pan.angle = angle
                else:
                    self.pan.angle = self.max_x
            ...
            while angle > move_to:
                ...
                if angle >= self.min_x:
                    ...
                    self.pan.angle = self.min_x
                time.sleep(self.servo_delay)
        self.moving_x = False

    def smooth_tilt(self, angle, move_to):
        print(f'tilt, angle - {angle}, move_to - {move_to}')
        if angle < move_to:
            while angle < move_to:
                angle += self.step_movement
                if angle <= self.max_y:
